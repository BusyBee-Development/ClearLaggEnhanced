...
public class NotificationManager {
    ...
    public void sendClearWarnings(int maxTime) {
        cancelWarnings();

        List<Integer> broadcastTimes = configManager.getIntegerList("notifications.broadcast-times");

        // Filter warnings to only those that fit within the lead time
        List<Integer> filteredTimes = broadcastTimes.stream()
                .filter(t -> t <= maxTime)
                .toList();

        if (filteredTimes.isEmpty() && maxTime <= 0) {
            warningTasks.add(scheduler.runLater(this::performClear, 1L));
            return;
        }

        for (int seconds : filteredTimes) {
        ...
        // Schedule the clear to happen exactly at the end of the maxTime period
