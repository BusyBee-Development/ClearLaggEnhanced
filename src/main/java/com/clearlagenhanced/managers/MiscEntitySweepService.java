...
public class MiscEntitySweepService {
    ...
    public MiscEntitySweepService(@NotNull ClearLaggEnhanced plugin, @NotNull ConfigManager cfg) {
        ...
        for (String s : cfg.getStringList("entity-clearing.whitelist")) {
            this.globalWhitelist.add(s.toUpperCase(Locale.ROOT));
        }
    ...
    private boolean exempt(@NotNull Entity e) {
        if (plugin.getEntityProtectionUtils().isProtected(e)) {
            return true;
        }
